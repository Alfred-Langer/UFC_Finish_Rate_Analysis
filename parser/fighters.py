...
Parses UFC Fighter data from saved Tapology Fighter Profile HTML pages using Playwright.
...
def parse_sex(page) -> str:
    ...
    for href in hrefs:
        if href and "championship-womens" in href:
            return "F"
        if href and "championship-mens" in href:
            return "M"
    return "NA"
...
def parse_fighter_profile(html_path: Path) -> tuple[Fighter, int]:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            name_div = page.locator("div#fighterPageHeader").locator("div.leading-tight").filter(
                has_not_text=re.compile(r"^\d+-\d+-\d+$")
            )
            if name_div.count() == 0:
                raise ValueError(f"Could not locate the name of the MMA Fighter profile: {file_url}")
            name = name_div.inner_text()

            # ── Standard Fighter Details Metadata ──────────────────────────────
            fighter_details_div = page.locator("div#standardDetails")

            # Extract and Parse Nickname
            nick_name_div = fighter_details_div.locator("div.items-center").filter(
                has=page.locator("strong", has_text=re.compile(r"^(Given )?Name:$"))
            )
            if nick_name_div.count() == 0:
                raise ValueError(f"Could not locate the nickname of the MMA Fighter profile: {file_url}")
            nick_name_div = parse_nick_name(nick_name_div.inner_text())

            # Extract and Parse MMA Record
            record_div = fighter_details_div.locator("div.items-center", has_text="MMA Record")
            if record_div.count() == 0:
                raise ValueError(f"Could not locate the record of the MMA Fighter profile: {file_url}")
            wins, losses, draws, no_contests, number_of_bouts = parse_record(record_div.inner_text())

            # Extract and Parse Age, Date of Birth, Height, and Reach
            age_height_dob_reach_div = fighter_details_div.locator("div#standardDetailsHeightAge")
            age_and_date_of_birth_div = age_height_dob_reach_div.locator("div.flex", has_text="Age")
            height_and_reach_div = age_height_dob_reach_div.locator("div.flex", has_text="Height")

            if age_height_dob_reach_div.count() == 0 or age_and_date_of_birth_div.count() == 0 or height_and_reach_div.count() == 0:
                logger.warning(f"Could not locate Age/Height for MMA Fighter profile: {file_url}")
                warnings += 1

            age, date_of_birth = parse_age_and_dob(age_and_date_of_birth_div.inner_text())
            height, reach = parse_height_and_reach(height_and_reach_div.inner_text())

            # Extract all MMA Bout divs and compute finish rate
            mma_bout_divs = page.locator("div[data-sport='mma'][data-division='pro'][data-sport='mma']:not([data-status='cancelled']):not([data-status='upcoming'])"
            ).filter(
                has=page.locator("span[title='Fighter Record Before Fight']")
            ).all()

            if len(mma_bout_divs) != number_of_bouts:
                logger.warning(f"MMA bout div count ({len(mma_bout_divs)}) does not match record ({number_of_bouts}) for: {name}")
                for mma_bout_div in mma_bout_divs:
                    logger.warning(f"  - Found bout with fighters: {mma_bout_div.locator('span[title=\"Fighter Record Before Fight\"]').inner_text()}")
                warnings += 1

            mma_bout_win_divs = [
                mma_bout_div.locator("div[data-status='win']")
                for mma_bout_div in mma_bout_divs
                if mma_bout_div.locator("div[data-status='win']").count() > 0
            ]
            num_finish_victories = 0
            for win_div in mma_bout_win_divs:
                win_method_div = win_div.locator("div.-rotate-90")
                if win_method_div.count() == 0:
                    logger.warning(f"Could not find method of victory for MMA Fighter: {name}")
                    warnings += 1
                    continue
                if "DEC" not in win_method_div.inner_text():
...
def parse_all_fighters():
    ...
    for file in sorted(FIGHTER_HTML_DIR.iterdir()):
        ...
        try:
            ...
            logger.info(f"✓ Successfully parsed: {fighter.name}" + (f" ({warnings} warnings)" if warnings else ""))
